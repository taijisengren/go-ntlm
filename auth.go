...
func (auth *ntlmAuth) Start(server *smtp.ServerInfo) (string, []byte, error) {
	if auth.initErr != nil {
		return "", nil, auth.initErr
	}
	if !server.TLS {
		var isNTLM bool
		for _, mechanism := range server.Auth {
...
func (auth *ntlmAuth) Next(fromServer []byte, more bool) ([]byte, error) {
	if !more {
		return nil, nil
	}
	...
	switch {
	case bytes.Equal(fromServer, []byte("NTLM supported")):
		return auth.InitialBytes()
	default:
		challengeMessage := fromServer
		return auth.NextBytes(challengeMessage)
	}
}
